...
# WRITE YOUR SOLUTION BELOW:
FIRST_OUTPUT_VALUE=$(bitcoin-cli -regtest decoderawtransaction "$BASE_TX" | jq '[.vout[0].value * 100000000] | .[0] | round')
...
VERIFY_PAYMENT=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address == "2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP") | .value')
check_cmd "Payment verification" "VERIFY_PAYMENT" "$VERIFY_PAYMENT"

VERIFY_CHANGE=$(echo "$DECODED_TX" | jq -r '.vout[] | select(.scriptPubKey.address == "bcrt1qg09ftw43jvlhj4wlwwhkxccjzmda3kdm4y83ht") | .value')
...
# WRITE YOUR SOLUTION BELOW:
CHANGE_OUTPUT_INDEX=$(bitcoin-cli -regtest decoderawtransaction "$RAW_TX" | jq -r '.vout[] | select(.scriptPubKey.address == "bcrt1qg09ftw43jvlhj4wlwwhkxccjzmda3kdm4y83ht") | .n')
...
# Convert to BTC
CHILD_SEND_BTC=$(echo "scale=8; $CHILD_SEND_AMOUNT / 100000000" | bc | sed 's/^\./0./')
...
# Convert to BTC
TIMELOCK_BTC=$(echo "scale=8; $TIMELOCK_AMOUNT / 100000000" | bc | sed 's/^\./0./')
